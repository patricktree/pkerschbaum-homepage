...
1. Instead of installing Node.js directly I like to use `nvm` to be able to have multiple versions of Node.js installed at the same time (you can read more here: <FancyAnchor href="https://patricktree.me/tidbits/multiple-vs-code-instances-with-separate-nodejs-versions">AUTOGENERATE</FancyAnchor>).  
...
- Install the extension <FancyAnchor href="https://marketplace.visualstudio.com/items?itemName=ms-vscode-remote.remote-wsl">WSL</FancyAnchor>, then you can run in an Ubuntu terminal `code .` to start VS Code in that folder.
  ...
  ```bash
  alias patricktree_dev='cd ~/workspace/patricktree-homepage && nvm use && code .'
  ```

  Read more about that here: <FancyAnchor href="https://patricktree.me/tidbits/multiple-vs-code-instances-with-separate-nodejs-versions">AUTOGENERATE</FancyAnchor>.
