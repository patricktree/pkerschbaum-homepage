...
```bash
alias dev_patricktree_homepage='cd ~/workspace/patricktree-homepage && nvm use && code .'
alias dev_document_generator='cd ~/workspace/react-pdf-document-generator && nvm use && code .'
```

Then I can run `dev_patricktree_homepage` and `dev_document_generator` to start two instances of VS Code each using the correct Node.js version!
...
```bash
$ dev_patricktree_homepage
Found '/home/patricktree/workspace/patricktree-homepage/.nvmrc' with version <16.19.0>
Now using node v16.19.0 (npm v8.19.3)
$ dev_document_generator
Found '/home/patricktree/workspace/react-pdf-document-generator/.nvmrc' with version <18.12.1>
